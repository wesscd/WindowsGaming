...
param (
  ...
  [switch]$SkipSecurityTweaks,
  [switch]$SkipBootTweaks,
  [switch]$SkipDebloat,
  [switch]$SkipNetworkTweaks,
  [switch]$Force,
...
$script:RunStats = [ordered]@{
  ...
  ExecutedTweaks  = New-Object System.Collections.Generic.List[string]
  SkippedTweaks   = New-Object System.Collections.Generic.List[string]
  FailedTweaks    = New-Object System.Collections.Generic.List[string]
  RiskScore       = 0.0
  EstimatedGain   = 0.0
}
...
function Write-Log {
  ...
  # Usar um único arquivo de log por execução para facilitar auditoria.
  if (-not $script:LogPath) {
    $script:LogPath = Join-Path -Path $logBasePath -ChildPath "$logBaseName`_$(Get-Date -Format 'yyyyMMdd_HHmmss')$logExtension"
  }
...
function Get-SystemOptimizationSnapshot {
  param (
    [string]$Stage
  )

  $cpu = Get-SafeValue { (Get-CimInstance Win32_Processor | Select-Object -First 1).Name }
  $gpu = Get-SafeValue { (Get-CimInstance Win32_VideoController | Select-Object -First 1).Name }
  $ramMb = Get-SafeValue { [math]::Round((Get-CimInstance Win32_ComputerSystem).TotalPhysicalMemory / 1MB) } 0
  $os = Get-SafeValue { (Get-CimInstance Win32_OperatingSystem | Select-Object -First 1) }
  $powerPlan = Get-SafeValue {
    $activePlan = powercfg /getactivescheme 2>$null
    if ($activePlan) { ($activePlan -join " ").Trim() } else { "Indisponível" }
  }
  $defenderStatus = Get-SafeValue {
    $mp = Get-MpComputerStatus
    "AMServiceEnabled=$($mp.AMServiceEnabled); RealTimeProtectionEnabled=$($mp.RealTimeProtectionEnabled)"
  }
  $networkProfiles = Get-SafeValue {
    (Get-NetConnectionProfile | ForEach-Object { "$($_.Name):$($_.NetworkCategory)" }) -join "; "
  }
  $runningServices = Get-SafeValue { (Get-Service | Where-Object Status -eq "Running").Count } 0
  $startupApps = Get-SafeValue {
    (Get-CimInstance Win32_StartupCommand).Count
  } 0
  $appxPackages = Get-SafeValue {
    (Get-AppxPackage -AllUsers).Count
  } 0
  $freeSystemDriveGb = Get-SafeValue {
  ...
  [ordered]@{
    ...
    OS                 = if ($os -is [string]) { $os } else { "$($os.Caption) $($os.Version) Build $($os.BuildNumber)" }
    CPU                = $cpu
    GPU                = $gpu
    RAM_MB             = $ramMb
    PowerPlan          = $powerPlan
    ...
    FreeSystemDrive_GB = $freeSystemDriveGb
  }
}
...
function Get-TweakMetadata {
  ...
  if ($downloads -contains $name) {
    ...
    $metadata.Risk = "High"
    $metadata.FpsWeight = 0.03
    $metadata.Description = "Baixa, instala ou executa componentes externos"
  }
  elseif ($security -contains $name) {
    $metadata.Category = "Security"
    $metadata.Risk = "Critical"
    $metadata.FpsWeight = 0.01
    $metadata.Description = "Altera configurações de segurança do Windows"
  }
  elseif ($boot -contains $name) {
    $metadata.Category = "Boot"
    $metadata.Risk = "Critical"
    $metadata.FpsWeight = 0.04
    $metadata.Description = "Altera boot, energia profunda ou mitigacoes"
  }
  elseif ($debloat -contains $name) {
    $metadata.Category = "Debloat"
    $metadata.Risk = "High"
    $metadata.FpsWeight = 0.03
    $metadata.Description = "Remove apps, pacotes provisionados ou integrações"
  }
  elseif ($network -contains $name) {
    $metadata.Category = "Network"
    $metadata.Risk = "High"
    $metadata.FpsWeight = 0.015
    $metadata.Description = "Altera pilha de rede, adaptadores ou perfil de conexão"
  }
  elseif ($performance -contains $name) {
    $metadata.Category = "Performance"
    $metadata.Risk = "Medium"
    $metadata.FpsWeight = 0.08
    $metadata.Description = "Ajuste voltado a desempenho e responsividade"
  }

  if ($name -eq "Finished" -or $name -eq "RequireAdmin" -or $name -eq "CreateRestorePoint") {
    $metadata.Risk = "Low"
    $metadata.FpsWeight = 0.0
  }

  [pscustomobject]$metadata
}

function Test-TweakAllowed {
  param (
    [pscustomobject]$Metadata
  )

  if ($SkipDownloads -and $Metadata.Category -eq "Download") { return "SkipDownloads ativo" }
  if ($SkipSecurityTweaks -and $Metadata.Category -eq "Security") { return "SkipSecurityTweaks ativo" }
  if ($SkipBootTweaks -and $Metadata.Category -eq "Boot") { return "SkipBootTweaks ativo" }
  if ($SkipDebloat -and $Metadata.Category -eq "Debloat") { return "SkipDebloat ativo" }
  if ($SkipNetworkTweaks -and $Metadata.Category -eq "Network") { return "SkipNetworkTweaks ativo" }

  switch ($ExecutionProfile) {
    "Essential" {
      if ($Metadata.Category -in @("Download", "Security", "Boot", "Debloat", "Network")) { return "Perfil Essential pula $($Metadata.Category)" }
    }
    "Gaming" {
      if ($Metadata.Category -in @("Security", "Boot", "Debloat")) { return "Perfil Gaming pula $($Metadata.Category)" }
    }
    "Advanced" {
      if ($Metadata.Category -eq "Security") { return "Perfil Advanced pula Security" }
    }
    default { }
  }
...
function Confirm-CriticalTweak {
  param (
    [pscustomobject]$Metadata
  )

  if ($Force -or $ExecutionProfile -eq "Extreme") { return $true }
  if ($Metadata.Risk -notin @("High", "Critical")) { return $true }

  Write-Colored "`nTweak: $($Metadata.Name)" "AmareloClaro"
  Write-Colored "Categoria: $($Metadata.Category) | Risco: $($Metadata.Risk)" "AmareloClaro"
  Write-Colored "Impacto: $($Metadata.Description)" "AmareloClaro"
  $answer = Read-Host "Deseja executar este tweak? (S/N)"
  $answer -match "^(?i)s$"
}

function Get-EstimatedFpsGain {
  param (
    [double]$GainScore,
    [int]$Reference
  )

  $cappedPercent = [math]::Min($GainScore, 0.18)
  $fpsGain = [math]::Round($Reference * $cappedPercent, 1)
  [ordered]@{
    Percent = [math]::Round($cappedPercent * 100, 1)
    FPS     = $fpsGain
    FinalFPS = [math]::Round($Reference + $fpsGain, 1)
  }
}
...
function Write-OptimizationReport {
  ...
  $script:RunStats.CompletedAt = Get-Date
  $estimate = Get-EstimatedFpsGain -GainScore $script:RunStats.EstimatedGain -Reference $ReferenceFPS

  $reportLines = New-Object System.Collections.Generic.List[string]
  $reportLines.Add("RELATORIO DE OTIMIZACAO - Windows Debloat and Gaming Tweaks")
  $reportLines.Add("RunId: $script:RunId")
  $reportLines.Add("Perfil: $ExecutionProfile")
  $reportLines.Add("DryRun: $DryRun")
  $reportLines.Add("Inicio: $($script:RunStats.StartedAt)")
  $reportLines.Add("Fim: $($script:RunStats.CompletedAt)")
  $reportLines.Add("")
  if ($script:InitialHealth) {
    $reportLines.Add("HEALTH CHECK INICIAL")
    $reportLines.Add("Status geral: $($script:InitialHealth.Status) - Nota $($script:InitialHealth.Score)/100")
    $reportLines.Add("Precisa de atencao em $($script:InitialHealth.IssueCount) item(ns): $($script:InitialHealth.HighCount) alta, $($script:InitialHealth.MediumCount) media, $($script:InitialHealth.LowCount) baixa")
    $reportLines.Add("Nota rapida: $($script:InitialHealth.Note)")
    if ($script:InitialHealth.Issues.Count -gt 0) {
      $script:InitialHealth.Issues |
        Sort-Object @{ Expression = { switch ($_.Severity) { "Alta" { 0 } "Media" { 1 } default { 2 } } } } |
        ForEach-Object { $reportLines.Add("- [$($_.Severity)] $($_.Item): $($_.Detail)") }
    }
    $reportLines.Add("")
  }
  $reportLines.Add("ANTES")
  Convert-SnapshotToText -Snapshot $Before | ForEach-Object { $reportLines.Add($_) }
  $reportLines.Add("")
  $reportLines.Add("DEPOIS")
  Convert-SnapshotToText -Snapshot $After | ForEach-Object { $reportLines.Add($_) }
  $reportLines.Add("")
  $reportLines.Add("TWEAKS EXECUTADOS ($($script:RunStats.ExecutedTweaks.Count))")
  $script:RunStats.ExecutedTweaks | ForEach-Object { $reportLines.Add("- $_") }
  $reportLines.Add("")
  $reportLines.Add("TWEAKS PULADOS ($($script:RunStats.SkippedTweaks.Count))")
  $script:RunStats.SkippedTweaks | ForEach-Object { $reportLines.Add("- $_") }
  $reportLines.Add("")
  $reportLines.Add("TWEAKS COM FALHA ($($script:RunStats.FailedTweaks.Count))")
  $script:RunStats.FailedTweaks | ForEach-Object { $reportLines.Add("- $_") }
  $reportLines.Add("")
  $reportLines.Add("ESTIMATIVA DE FPS")
  $reportLines.Add("Referencia informada: $ReferenceFPS FPS")
  $reportLines.Add("Ganho medio estimado: +$($estimate.FPS) FPS (+$($estimate.Percent)%)")
  $reportLines.Add("FPS medio estimado apos ajustes: $($estimate.FinalFPS) FPS")
  $reportLines.Add("Observacao: esta estimativa e heuristica. Para medir ganho real, rode benchmark do mesmo jogo/cena antes e depois.")
  $reportLines.Add("")
  $reportLines.Add("Log detalhado: $script:LogPath")
  $reportLines.Add("Backup de auditoria: $script:BackupRoot")

  Set-Content -Path $script:ReportPath -Value $reportLines -Encoding UTF8

  Write-Colored "`nRelatorio salvo em: $script:ReportPath" "VerdeClaro"
  Write-Colored "Estimativa: +$($estimate.FPS) FPS em media (+$($estimate.Percent)%) usando referencia de $ReferenceFPS FPS." "VerdeClaro"
}
...
function Initialize-OptimizationBackup {
  ...
  Write-Log "Criando backup de auditoria em $script:BackupRoot" -Level "INFO" -ConsoleOutput

  Get-SafeValue {
    bcdedit /enum all | Out-File -FilePath (Join-Path $script:BackupRoot "bcdedit_enum_all.txt") -Encoding UTF8
  } | Out-Null

  Get-SafeValue {
    netsh int tcp show global | Out-File -FilePath (Join-Path $script:BackupRoot "netsh_tcp_global.txt") -Encoding UTF8
  } | Out-Null

  Get-SafeValue {
    powercfg /list | Out-File -FilePath (Join-Path $script:BackupRoot "power_plans.txt") -Encoding UTF8
  } | Out-Null

  Get-SafeValue {
    Get-Service | Select-Object Name, DisplayName, Status, StartType |
      Export-Csv -Path (Join-Path $script:BackupRoot "services.csv") -NoTypeInformation -Encoding UTF8
  } | Out-Null

  Get-SafeValue {
    Get-AppxPackage -AllUsers | Select-Object Name, PackageFullName, Version |
      Export-Csv -Path (Join-Path $script:BackupRoot "appx_packages.csv") -NoTypeInformation -Encoding UTF8
  } | Out-Null

  Get-SafeValue {
    Get-CimInstance Win32_StartupCommand | Select-Object Name, Command, Location, User |
      Export-Csv -Path (Join-Path $script:BackupRoot "startup_commands.csv") -NoTypeInformation -Encoding UTF8
  } | Out-Null

  $registryBackups = @(
    @{ Key = "HKLM\SOFTWARE\Policies\Microsoft\Windows"; File = "HKLM_Policies_Microsoft_Windows.reg" },
    @{ Key = "HKLM\SYSTEM\CurrentControlSet\Control"; File = "HKLM_System_Control.reg" },
    @{ Key = "HKLM\SYSTEM\CurrentControlSet\Services\Tcpip"; File = "HKLM_Tcpip.reg" },
    @{ Key = "HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer"; File = "HKCU_Explorer.reg" },
    @{ Key = "HKCU\System\GameConfigStore"; File = "HKCU_GameConfigStore.reg" }
  )

  foreach ($item in $registryBackups) {
    $destination = Join-Path $script:BackupRoot $item.File
    Get-SafeValue {
      & reg.exe export $item.Key $destination /y | Out-Null
    } | Out-Null
  }

  Write-Log "Backup de auditoria finalizado em $script:BackupRoot" -Level "INFO" -ConsoleOutput
}

function Get-SystemHealthCheck {
  $issues = New-Object System.Collections.Generic.List[object]

  $os = Get-SafeValue { Get-CimInstance Win32_OperatingSystem | Select-Object -First 1 } $null
  $computer = Get-SafeValue { Get-CimInstance Win32_ComputerSystem | Select-Object -First 1 } $null
  $systemDrive = Get-SafeValue { Get-CimInstance Win32_LogicalDisk -Filter "DeviceID='$env:SystemDrive'" } $null
  $physicalDisks = Get-SafeValue { Get-PhysicalDisk } @()
  $startupApps = Get-SafeValue { (Get-CimInstance Win32_StartupCommand).Count } 0
  $powerPlan = Get-SafeValue {
    $activePlan = powercfg /getactivescheme 2>$null
    if ($activePlan) { ($activePlan -join " ").Trim() } else { "" }
  } ""
  $defender = Get-SafeValue { Get-MpComputerStatus } $null
  $pageFile = Get-SafeValue {
    Get-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management" -Name "PagingFiles", "AutomaticManagedPagefile" -ErrorAction Stop
  } $null
  $pendingReboot = Get-SafeValue {
    (Test-Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Component Based Servicing\RebootPending") -or
    (Test-Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Auto Update\RebootRequired")
  } $false
  $gameDvrEnabled = Get-SafeValue {
    (Get-ItemProperty -Path "HKCU:\System\GameConfigStore" -Name "GameDVR_Enabled" -ErrorAction Stop).GameDVR_Enabled
  } $null
  $hags = Get-SafeValue {
    (Get-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Control\GraphicsDrivers" -Name "HwSchMode" -ErrorAction Stop).HwSchMode
  } $null
  $networkAdapters = Get-SafeValue {
    Get-NetAdapter | Where-Object { $_.Status -eq "Up" -and $_.InterfaceDescription -notmatch "Loopback" }
  } @()

  if ($systemDrive) {
    $freeGb = [math]::Round($systemDrive.FreeSpace / 1GB, 1)
    $totalGb = [math]::Round($systemDrive.Size / 1GB, 1)
    $freePercent = if ($systemDrive.Size -gt 0) { [math]::Round(($systemDrive.FreeSpace / $systemDrive.Size) * 100, 1) } else { 0 }
    if ($freePercent -lt 10 -or $freeGb -lt 20) {
      $issues.Add([pscustomobject]@{ Severity = "Alta"; Item = "Pouco espaço livre no disco do sistema"; Detail = "$freeGb GB livres de $totalGb GB ($freePercent%). Libere espaço antes de otimizações pesadas." }) | Out-Null
    }
    elseif ($freePercent -lt 20 -or $freeGb -lt 50) {
      $issues.Add([pscustomobject]@{ Severity = "Media"; Item = "Espaço livre limitado no disco do sistema"; Detail = "$freeGb GB livres de $totalGb GB ($freePercent%). Ideal manter folga para updates, shaders e pagefile." }) | Out-Null
    }
  }
  ...
  if ($computer) {
    ...
    if ($ramGb -lt 8) {
      $issues.Add([pscustomobject]@{ Severity = "Alta"; Item = "RAM abaixo do recomendado"; Detail = "$ramGb GB detectados. Jogos atuais tendem a sofrer com stutter abaixo de 8 GB." }) | Out-Null
    }
    elseif ($ramGb -lt 16) {
      $issues.Add([pscustomobject]@{ Severity = "Media"; Item = "RAM moderada para jogos atuais"; Detail = "$ramGb GB detectados. 16 GB costuma ser o ponto mais confortável." }) | Out-Null
    }
  }

  foreach ($disk in @($physicalDisks)) {
    if ($disk.HealthStatus -and $disk.HealthStatus -ne "Healthy") {
      $issues.Add([pscustomobject]@{ Severity = "Alta"; Item = "Saúde de disco requer atenção"; Detail = "$($disk.FriendlyName): HealthStatus=$($disk.HealthStatus), OperationalStatus=$($disk.OperationalStatus)." }) | Out-Null
    }
  }

  if ($defender) {
    if (-not $defender.RealTimeProtectionEnabled) {
      $issues.Add([pscustomobject]@{ Severity = "Alta"; Item = "Proteção em tempo real do Defender desligada"; Detail = "O sistema fica mais exposto durante downloads e execução de ferramentas externas." }) | Out-Null
    }
    if ($defender.AntivirusSignatureAge -gt 7) {
      $issues.Add([pscustomobject]@{ Severity = "Media"; Item = "Assinaturas do Defender antigas"; Detail = "Assinaturas com $($defender.AntivirusSignatureAge) dias. Atualize antes de baixar executáveis." }) | Out-Null
    }
  }

  if ($pendingReboot) {
    $issues.Add([pscustomobject]@{ Severity = "Media"; Item = "Reinicialização pendente"; Detail = "Finalize updates/reinicie antes de aplicar muitos tweaks para evitar estado inconsistente." }) | Out-Null
  }

  if ($startupApps -gt 25) {
    $issues.Add([pscustomobject]@{ Severity = "Media"; Item = "Muitos programas na inicialização"; Detail = "$startupApps entradas detectadas. Isso pode aumentar boot time e consumo em segundo plano." }) | Out-Null
  }

  if ($powerPlan -and $powerPlan -notmatch "Alto|High|Ultimate|Desempenho|Performance") {
    $issues.Add([pscustomobject]@{ Severity = "Baixa"; Item = "Plano de energia não focado em desempenho"; Detail = "Plano ativo: $powerPlan. Para jogos, um perfil de alto desempenho pode ajudar." }) | Out-Null
  }

  if ($pageFile -and $pageFile.AutomaticManagedPagefile -eq 0) {
    $issues.Add([pscustomobject]@{ Severity = "Baixa"; Item = "Pagefile gerenciado manualmente"; Detail = "Pagefile manual detectado. Pode ser bom, mas vale conferir tamanho e unidade configurada." }) | Out-Null
  }

  if ($gameDvrEnabled -eq 1) {
    $issues.Add([pscustomobject]@{ Severity = "Baixa"; Item = "Game DVR ativo"; Detail = "Captura em segundo plano pode impactar alguns jogos competitivos." }) | Out-Null
  }

  if ($null -eq $hags) {
    $issues.Add([pscustomobject]@{ Severity = "Baixa"; Item = "HAGS sem configuração explícita"; Detail = "Agendamento de GPU por hardware não possui valor definido no registro." }) | Out-Null
  }

  if (-not $networkAdapters -or @($networkAdapters).Count -eq 0) {
    $issues.Add([pscustomobject]@{ Severity = "Media"; Item = "Nenhum adaptador de rede ativo detectado"; Detail = "Otimizações/downloads podem falhar se a conexão não estiver ativa." }) | Out-Null
  }

  $high = @($issues | Where-Object Severity -eq "Alta").Count
  $medium = @($issues | Where-Object Severity -eq "Media").Count
  $low = @($issues | Where-Object Severity -eq "Baixa").Count
  $score = [math]::Max(0, 100 - ($high * 18) - ($medium * 9) - ($low * 4))
  $status = if ($score -ge 85) { "Bom" } elseif ($score -ge 70) { "Atenção leve" } elseif ($score -ge 50) { "Precisa de atenção" } else { "Crítico" }
  $note = if ($issues.Count -eq 0) {
    "Nenhum ponto relevante encontrado antes das otimizações."
  }
  else {
    $topIssue = ($issues | Sort-Object @{ Expression = { switch ($_.Severity) { "Alta" { 0 } "Media" { 1 } default { 2 } } } } | Select-Object -First 1)
    "Prioridade: $($topIssue.Item). $($topIssue.Detail)"
  }
...
function Get-HealthCheckIntroLines {
  param (
  ...
  if ($Health.Issues.Count -gt 0) {
    ...
    $Health.Issues |
      ...
      ForEach-Object {
        $lines.Add("- [$($_.Severity)] $($_.Item): $($_.Detail)")
      }
  }
...
function Show-Intro {
  ...
  $script:InitialHealth = Get-SystemHealthCheck
  $healthLines = Get-HealthCheckIntroLines -Health $script:InitialHealth
...
function RequireAdmin {
  if (-not ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    ...
    if ($SkipDownloads) { $argumentList += "-SkipDownloads" }
    if ($SkipSecurityTweaks) { $argumentList += "-SkipSecurityTweaks" }
    if ($SkipBootTweaks) { $argumentList += "-SkipBootTweaks" }
    if ($SkipDebloat) { $argumentList += "-SkipDebloat" }
    if ($SkipNetworkTweaks) { $argumentList += "-SkipNetworkTweaks" }
...
function Execute-BatchScript {
  ...
  try {
    ...
    $localPath = "$env:TEMP\baraotech.bat"
...
function Invoke-TweakSafely {
  ...
  $metadata = Get-TweakMetadata -Tweak $Tweak
  $tweakName = $metadata.Name

  Write-Log "Preparando tweak: $tweakName (Categoria=$($metadata.Category), Risco=$($metadata.Risk), Passo $CurrentStep de $TotalSteps)" -Level "INFO" -ConsoleOutput
  Show-ProgressBar -CurrentStep $CurrentStep -TotalSteps $TotalSteps -TaskName $tweakName

  if ($tweakName -eq "Finished") {
    $script:RunStats.SkippedTweaks.Add("$tweakName - executado depois do relatorio") | Out-Null
    return "Deferred"
  }

  $skipReason = Test-TweakAllowed -Metadata $metadata
  if ($skipReason) {
    Write-Log "Tweak $tweakName pulado: $skipReason" -Level "WARNING" -ConsoleOutput
    $script:RunStats.SkippedTweaks.Add("$tweakName - $skipReason") | Out-Null
    return "Skipped"
  }

  if (-not (Confirm-CriticalTweak -Metadata $metadata)) {
    Write-Log "Tweak $tweakName pulado por escolha do usuario." -Level "WARNING" -ConsoleOutput
    $script:RunStats.SkippedTweaks.Add("$tweakName - recusado pelo usuario") | Out-Null
    return "Skipped"
  }

  if ($DryRun) {
    Write-Log "DryRun ativo: $tweakName seria executado, mas nenhuma alteracao foi aplicada." -Level "INFO" -ConsoleOutput
    $script:RunStats.SkippedTweaks.Add("$tweakName - DryRun") | Out-Null
    return "DryRun"
  }
  ...
  if ($tweakFunctions.ContainsKey($tweakName)) {
    try {
      ...
      Write-Log "Tweak $tweakName concluido com sucesso." -Level "INFO" -ConsoleOutput
      $script:RunStats.ExecutedTweaks.Add("$tweakName [$($metadata.Category)/$($metadata.Risk)]") | Out-Null
      $script:RunStats.RiskScore += switch ($metadata.Risk) {
        "Low" { 0.25 }
        "Medium" { 0.5 }
        "High" { 0.75 }
        "Critical" { 1.0 }
      ...
      $script:RunStats.EstimatedGain += [double]$metadata.FpsWeight
      return "Executed"
    }
    catch {
      ...
      $script:RunStats.FailedTweaks.Add("$tweakName - $_") | Out-Null
      return "Failed"
    }
  }
  else {
    Write-Log "Tweak nao encontrado: $Tweak" -Level "WARNING" -ConsoleOutput
    Write-Colored "`nTweak nao encontrado: $Tweak" -Color "VermelhoClaro"
    $script:RunStats.FailedTweaks.Add("$tweakName - funcao nao encontrada") | Out-Null
    return "Missing"
  }
}

# Executar introdução e capturar estado inicial
Show-Intro
$beforeSnapshot = Get-SystemOptimizationSnapshot -Stage "Antes"
Initialize-OptimizationBackup

# Executar os tweaks com barra de progresso, controle de risco e relatorio
...
foreach ($tweak in $tweaks) {
  $currentStep++
  $result = Invoke-TweakSafely -Tweak $tweak -CurrentStep $currentStep -TotalSteps $totalTweaks
  if ($result -eq "Deferred") {
    $finishedDeferred = $true
  }
  Start-Sleep -Milliseconds 100 # Pequena pausa para visualização
}

Write-Host "" # Nova linha após o progresso
$afterSnapshot = Get-SystemOptimizationSnapshot -Stage "Depois"
Write-OptimizationReport -Before $beforeSnapshot -After $afterSnapshot

if ($finishedDeferred -and -not $DryRun) {
  Finished
}
